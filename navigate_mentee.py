...
def get_otp_from_email():
    ...
    PASSWORD = os.getenv("SENDER_PASSWORD2")
    try:
...
def send_failure_email():
    ...
    SENDER_PASSWORD = os.getenv("SENDER_PASSWORD2")
...
if __name__ == "__main__":
    urls = [os.getenv("CRM_URL2"), os.getenv("CRM_URL4")]
    with ThreadPoolExecutor(max_workers=len(urls)) as executor:
        # Add incremental delay for each URL
        delays = [i * 30 for i in range(len(urls))]  # 40 seconds apart
